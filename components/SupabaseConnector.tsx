...
export function SupabaseConnector({ onSyncComplete, localTransacciones }: SupabaseConnectorProps) {
  ...
  const [showAdvanced, setShowAdvanced] = useState(!config.isConfigured);

  // Sync state if config changes dynamically
  useEffect(() => {
    setShowAdvanced(!config.isConfigured);
  }, [config.isConfigured]);
...
`;
  ...
  return (
    <div className="glass-panel rounded-3xl p-6 flex flex-col gap-6 shadow-xl w-full" id="supabase-connector-panel">
        ...
        <div className="flex items-center gap-2">
          {config.isConfigured && (
            <button
              ...
              className="text-xs font-mono text-[#4edea3] border border-[#4edea3]/20 hover:bg-[#4edea3]/10 px-3 py-1.5 rounded-xl transition-all cursor-pointer bg-transparent outline-none"
            >
      ...
      {config.isConfigured && !showAdvanced ? (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-[#131b2e]/30 border border-[#4edea3]/20 rounded-2xl p-4 animate-fadeIn">
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-[#4edea3] animate-pulse" />
              <span className="text-xs font-semibold text-white">Sincronización en la Nube Activa</span>
            </div>
            <span className="text-[10px] text-[#bbcabf]/60 font-mono truncate max-w-[280px] sm:max-w-lg block">
              URL: {config.url}
            </span>
          </div>
          <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
            <button
              ...
              disabled={isSyncing}
              className="w-full sm:w-auto bg-[#4edea3]/10 hover:bg-[#4edea3]/20 border border-[#4edea3]/30 text-[#4edea3] text-xs font-mono font-bold uppercase py-2 px-4 rounded-xl flex items-center justify-center gap-1.5 transition-all outline-none"
            >
              {isSyncing ? (
                <>
